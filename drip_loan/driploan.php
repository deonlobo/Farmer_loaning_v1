<?php 
	...
	$sql = "SELECT * FROM LOAN L,BORROW BR,BANK B 
	        WHERE L.LoanID=BR.LoanID AND BR.BankID=B.BankID AND Lstatus='Pending' AND Approve='Approved' AND BR.LoanType='Drip Irrigation Loan'
...
<html>
    ...
    <?php if($loans){ ?>
		<h4 class="center lightgrey" style="margin-top:50px;margin-bottom: 50px;">Loans Applied</h4>
    <?php }?>
	<div class="container">
		<div class="row">
			<?php if($loans){ ?>
			...
			<?php }else {?>
				      <h4 class="center lightgrey" style="margin-top:50px;margin-bottom: 50px;">No Drip Irrigation Loans Applied</h4>
		    <?php }?>
